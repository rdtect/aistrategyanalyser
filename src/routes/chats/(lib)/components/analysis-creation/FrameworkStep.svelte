<script lang="ts">
  ...
  // Default icon to use when no icon is specified
  const DefaultIcon = IconBox;
</script>
...
<div class="space-y-4 step-content">
  <div class="card bg-primary-500/10 p-4 rounded-container-token mb-4">
    ...
    <p class="text-sm">
      Choose a framework to guide your strategic analysis. Each framework
  ...
  {#if allFrameworkOption}
    <div class="mb-4">
      <button
        type="button"
        class="w-full p-4 rounded-container-token framework-card flex gap-4 items-center {selectedFrameworkId ===
        allFrameworkOption.id
          ? 'selected'
          : ''}"
      ...
      >
        <div class="bg-primary-500/20 p-3 rounded-full">
          <DefaultIcon size={24} class="text-primary-700" />
        </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
    {#each frameworkOptions.filter((opt: FrameworkOption) => opt.id !== "all") as option (option.id)}
      <button
        type="button"
        class="p-4 rounded-container-token framework-card {selectedFrameworkId ===
        option.id
          ? 'selected'
      ...
      >
        <div class="flex items-center gap-3 mb-2">
          <div
            class="bg-primary-500/20 p-2 rounded-full {option.disabled
              ? '!bg-surface-500/20'
              : ''}"
          >
            <DefaultIcon
              size={20}
              class={option.disabled ? "text-surface-700" : "text-primary-700"}
            />
          </div>
    ...
    <p class="text-sm">
      {#if selectedFrameworkId}
        {@const selectedOption = frameworkOptions.find(
          (opt: FrameworkOption) => opt.id === selectedFrameworkId,
        )}

...
{#if isUserMessage}
  ...
  <div
    class="grid grid-cols-[1fr_auto] gap-2 mb-4"
    transition:slide|local={{ duration: 300, easing: sineIn }}
  >
    <div class="card p-4 rounded-tr-none space-y-2 glass-primary">
      <header class="flex justify-between items-center">
        <p class="font-bold">You</p>
        <small class="opacity-50">{formatTime(message.timestamp)}</small>
      </header>
      <div
        class="message-content prose prose-sm dark:prose-invert max-w-none break-words"
      >
    ...
    <div class="flex items-start">
      <!-- User Avatar Wrapper -->
      <div
        class="w-12 h-12 rounded-full overflow-hidden flex items-center justify-center glass-primary"
      >
        <Avatar name="User">
          <IconUser size="24" />
        </Avatar>
...
{:else if isAiMessage}
  ...
  >
    ...
    <div class="card p-4 rounded-tl-none space-y-2 glass-surface">
      <header class="flex justify-between items-center">
...
{:else if isSystemMessage}
  ...
  <div
    class="flex justify-center mb-4 text-center"
    transition:slide|local={{ duration: 300, easing: sineIn }}
  >
    <div
      class="message-content prose prose-xs dark:prose-invert text-surface-500 italic break-words"
    >
      {@html formattedContent}
    </div>
  </div>

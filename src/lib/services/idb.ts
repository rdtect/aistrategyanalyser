...
// Import canonical types from the core directory
...
// Service state - use plain variables in a .ts module
let db: IDBPDatabase | null = null;
let isInitialized = false;
let fallbackStorage: Map<string, Chat> = new Map();
  ...
  try {
    ...
    try {
      ...
      if (result) {
        ...
        console.error(
          "IndexedDB read/write test failed: couldn't read test data",
        );
      }
    } catch (testError) {
...
async function getChat(id: string): Promise<Chat | null> {
  if (!isInitialized) {
    console.log(
      `IDB: Service not initialized, reinitializing before getting chat ${id}`,
    );
    await initDB();
  }
  ...
  try {
    if (db) {
      ...
      const chat = (await db.get(CHATS_STORE, id)) as Chat | undefined;
      ...
      if (!chat) {
        ...
        if (browser && typeof localStorage !== "undefined") {
          try {
            const fallbackJson =
              localStorage.getItem(`chat_fallback_${id}`) ||
              localStorage.getItem(`chat_backup_${id}`);
            if (fallbackJson) {
              console.log(`IDB: Found chat ${id} in localStorage fallback`);
              const fallbackChat = JSON.parse(fallbackJson) as Chat;

              // If found in localStorage, try to restore it to IndexedDB
              if (fallbackChat && fallbackChat.id === id) {
                console.log(
                  `IDB: Restoring chat ${id} from localStorage to IndexedDB`,
                );
                try {
                  await db.put(CHATS_STORE, fallbackChat);
                  console.log(
                    `IDB: Successfully restored chat ${id} to IndexedDB`,
                  );
                } catch (restoreErr) {
                  console.error(
                    `IDB: Failed to restore chat to IndexedDB:`,
                    restoreErr,
                  );
                }
                return fallbackChat;
            ...
            console.error(
              `IDB: Error checking localStorage fallback:`,
              lsError,
            );
          }
        }
    ...
    if (browser && typeof localStorage !== "undefined") {
      try {
        ...
        if (fallbackJson) {
          console.log(
            `IDB: Found chat ${id} in localStorage during error recovery`,
          );
          return JSON.parse(fallbackJson) as Chat;
        }
      } catch (lsError) {
        console.error(
          `IDB: Error checking localStorage during recovery:`,
          lsError,
        );
      }
    }
...
async function saveChat(chat: Chat): Promise<void> {
  if (!isInitialized) {
    console.log(
      `IDB: Service not initialized, reinitializing before saving chat ${chat.id}`,
    );
    await initDB();
  }
  ...
  try {
    if (db) {
      ...
      try {
        ...
        console.log(
          `IDB: Read test before save ${chat.id}: ${existingChat ? "successful" : "not found (new chat)"}`,
        );
      } catch (readErr) {
      ...
      try {
        ...
        if (verifyChat) {
          ...
          console.error(
            `IDB: Verify after save for ${chat.id} failed - chat not found after save`,
          );
          throw new Error(
            "Chat not found after save - database may be inconsistent",
          );
        }
      } catch (verifyErr) {

<script lang="ts">
	import { chatManager, chats, currentChatId } from './Chat.svelte.ts';
	import ChatListIcon from 'lucide-svelte/icons/message-circle-code';
	import type { Chat } from '$lib/data/sampleChats';
</script>

<div class="border-surface-200-800 hidden h-full grid-rows-[auto_1fr_auto] overflow-hidden border-r-[1px] lg:grid">
	<!-- Header -->
		...
		<div class="flex flex-col space-y-1">
			{#each $chats as chat}
				<button
					...
					onclick={() => chatManager.switchChat(chat.id)}
				>

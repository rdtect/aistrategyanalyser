...
import { IDBService } from "$lib/services/IDBService.svelte";
...
function createChatStore() {
  ...
  async function saveToStorage(newChats: Record<string, Chat>) {
    ...
    try {
      ...
      for (const chat of Object.values(newChats)) {
        await IDBService.saveChat(chat);
      }
  ...
  async function loadFromStorage(): Promise<Record<string, Chat>> {
    ...
    try {
      ...
      const chatArray = await IDBService.getAllChats();
  ...
  async function init(initialChats: Chat[] = []) {
    ...
    try {
      ...
      if (initialChats && initialChats.length > 0) {
        console.log(
          `Checking ${initialChats.length} initial chats for storage`,
        );
        for (const chat of initialChats) {
          // Check if we have this chat already before saving
          const existingChat = await IDBService.getChat(chat.id);
          if (!existingChat) {
            ...
            await IDBService.saveChat(chatToSave);
  ...
  const store = {
    ...
    async sendMessage(content: string): Promise<boolean> {
      ...
      try {
        // Save to storage
        await IDBService.addMessageToChat(activeChatId, message);
        return true;
      } catch (e) {
    ...
    // Subscribe function for store compatibility
    subscribe(callback: (value: any) => void) {
      const unsubscribe = writable({
        getValue: () => ({
          chats,
          activeChatId,
...
// Helper functions that use the store instance

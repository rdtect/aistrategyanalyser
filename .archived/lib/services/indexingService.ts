...
import categoryQuestionsData from "$lib/data/category_question_prompts.json";

// Create a properly typed categoryQuestions constant
const categoryQuestions = categoryQuestionsData as Record<
  string,
  Array<{
    id: string;
    question: string;
...
export async function indexDocuments(documents: any[]) {
  const results = await Promise.all(
    documents.map((doc) => storeDocumentWithEmbedding(doc)),
  );
  return results;
}

  ...
  {#if chatLogic.activeChat}
    <div class="border-b border-surface-700 p-1 flex justify-between items-center backdrop-blur-sm bg-surface-800/20 px-8">
      ...
      <div class="flex items-center gap-2">
        <!-- Memory Extractor -->
        {#if chatLogic.activeChat && chatLogic.messages.length > 0}
          <MemoryExtractor 
            chat={{
              id: chatLogic.activeChat.id,
              name: chatLogic.activeChat.name,
              company: chatLogic.activeChat.context?.company || '',
              industry: chatLogic.activeChat.context?.industry || '',
              region: chatLogic.activeChat.context?.region || '',
              context: chatLogic.activeChat.context?.additionalInfo || '',
              createdAt: typeof chatLogic.activeChat.createdAt === 'string' 
                ? chatLogic.activeChat.createdAt 
                : new Date(chatLogic.activeChat.createdAt || 0).toISOString(),
              updatedAt: typeof chatLogic.activeChat.updatedAt === 'string' 
                ? chatLogic.activeChat.updatedAt 
                : new Date(chatLogic.activeChat.updatedAt || 0).toISOString(),
              messages: chatLogic.messages.map(m => {
                // Map status to valid values only
                let status: "sending" | "sent" | "error" | "streaming" | undefined;
                if (m.status === "sending" || m.status === "sent" || m.status === "error" || m.status === "streaming") {
                  status = m.status;
                }
